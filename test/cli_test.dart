...
import 'package:ricky_cli/commands/splash/ios/ios_splash_controller.dart';
...
void main() {
  ...
  group('IOS splash creation tests', () {
    ...
    test('can execute successfully with provided background and ios folder exist', () {
      expectLater(IOSSplashController(backgroundColor: '#000000', customSourceImage: sourceImage!).execute(), completion(true));
    });
  });

  group('Structure templates tests', () {
    test('can read in memory structure config', () async {
      final inMemoryTemplate = Template(source: InMemoryConfig(content: kBlocTemplateSpecification));

      final structure = await inMemoryTemplate.structure;

      final expectedStructure = kBlocTemplateSpecification.map((element) => TemplateElement(path: element.path, tag: '')).toList();
    ...
    test('can read in file structure config', () async {
      ...
      final expectedStructure = kBlocTemplateSpecification;
